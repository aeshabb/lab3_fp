...
let default_config = { methods = []; step = 1.0; window_size = 2 }
...
let parse_args () =
  ...
  let spec_list =
    [
      ( "--linear",
        Arg.Unit (fun () -> methods_ref := Linear :: !methods_ref),
        "Use linear interpolation" );
      ( "--newton",
        Arg.Unit (fun () -> methods_ref := Newton :: !methods_ref),
        "Use Newton interpolation" );
      ("--step", Arg.Set_float step_ref, "Set interpolation step (default: 1.0)");
      ( "-n",
        Arg.Set_int window_size_ref,
        "Set window size for interpolation (default: 2)" );
    ]
  in
  ...
  let final_methods =
    if List.length methods = 0 then [ Linear ] else List.rev methods
  in

  { methods = final_methods; step; window_size }

let method_name = function Linear -> "linear" | Newton -> "newton"
